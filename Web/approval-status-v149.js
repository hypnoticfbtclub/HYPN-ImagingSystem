(() => {
  ...
  style.id = 'hypn-approval-v149-style';
  style.textContent = `
    #collabPosterGrid .poster-preview{position:relative}
    #collabPosterGrid .hypn-approval-overlay{position:absolute;z-index:20;top:12px;right:12px;display:inline-flex;align-items:center;gap:7px;padding:9px 12px;border-radius:999px;font-size:11px;font-weight:950;letter-spacing:.02em;border:1px solid;box-shadow:0 8px 24px rgba(0,0,0,.32);backdrop-filter:blur(8px);pointer-events:none}
    #collabPosterGrid .hypn-approval-overlay.approved{color:#8ff0b7;background:rgba(15,52,33,.94);border-color:#347653}
    #collabPosterGrid .hypn-approval-overlay.unapproved{color:#ff9aa2;background:rgba(68,20,27,.95);border-color:#963b49}
    #collabPosterGrid .hypn-approval-icon{width:20px;height:20px;border-radius:50%;display:inline-grid;place-items:center;font-size:13px;font-weight:950;line-height:1}
  ...
  function decorateCollaboratorPoster(card) {
    ...
    if (textSaysUnapproved(originalStatus)) return setState(preview, badge, false);
  ...
  function apply() {
    ...
    if (footer) footer.textContent = `HYPN Imaging System V${VERSION} • ADMIN publica directo • colaboradores requieren aprobación`;
  }
