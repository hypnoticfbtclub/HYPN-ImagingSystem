(() => {
  ...
  style.textContent = `
    ...
    .hypn-password-current{display:grid;grid-template-columns:1fr auto;gap:8px;align-items:end}
    ...
    .hypn-password-missing{color:var(--warn);font-size:12px}
    @media(max-width:850px){.hypn-user-grid,.hypn-user-meta,.hypn-permissions,.hypn-password-row,.hypn-password-current{grid-template-columns:1fr}}
  `;
  ...
  loadUsers = async function () {
    ...
    try {
      ...
      for (const user of data.users) {
        ...
        const hasVisiblePassword = user.password_visible && typeof user.password_value === 'string';
        const currentPassword = hasVisiblePassword ? user.password_value : '';
        ...
        card.innerHTML = `
          ...
          <div class="hypn-user-grid">
            ...
            <div class="hypn-password-current">
              <label>Clave actual
                <input class="current-password" type="password" value="${esc(currentPassword)}" readonly placeholder="${hasVisiblePassword ? '' : 'NO DISPONIBLE - CAMBIA LA CLAVE UNA VEZ'}">
              </label>
              <button class="ghost small show-current" ${hasVisiblePassword ? '' : 'disabled'}>MOSTRAR</button>
            </div>
          </div>

          ${hasVisiblePassword
            ? '<div class="hypn-user-note">Esta es exactamente la clave que el ADMIN asignó a este colaborador.</div>'
            : '<div class="hypn-password-missing">Este usuario fue creado antes de V1.4.9. Su clave anterior no puede recuperarse. Cámbiala una vez y desde ese momento sí quedará visible para el ADMIN.</div>'}

          <div class="hypn-password-row">
            <label>Nueva contraseña
              <input class="new-password" type="password" placeholder="Escribe una nueva clave" maxlength="256">
            </label>
            <button class="ghost small show-new">MOSTRAR</button>
        ...
        const currentPasswordInput = card.querySelector('.current-password');
        const newPasswordInput = card.querySelector('.new-password');
        const showCurrentBtn = card.querySelector('.show-current');
        const showNewBtn = card.querySelector('.show-new');

        showCurrentBtn.onclick = () => {
          if (!hasVisiblePassword) return;
          const visible = currentPasswordInput.type === 'text';
          currentPasswordInput.type = visible ? 'password' : 'text';
          showCurrentBtn.textContent = visible ? 'MOSTRAR' : 'OCULTAR';
        };

        showNewBtn.onclick = () => {
          const visible = newPasswordInput.type === 'text';
          newPasswordInput.type = visible ? 'password' : 'text';
          showNewBtn.textContent = visible ? 'MOSTRAR' : 'OCULTAR';
        };
        ...
        card.querySelector('.save-password').onclick = async () => {
          ...
          try {
            ...
            msg('usersMsg', `Clave de ${usernameInput.value} actualizada. Ahora ya puede verla el ADMIN y usarla el colaborador.`, 'ok');
            await loadUsers();
          } catch (e) {
